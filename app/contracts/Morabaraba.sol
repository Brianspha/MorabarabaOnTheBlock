...
{
    emit CreatedNewGame("Successfully created new game.");
}
...
function NewGame(address one,address two) public returns (bool success)
{
  ...
  currentGameSession=Morabaraba(p1,p2,brd,one,two,SessionIdCount++,true);
  success =true;
...
function MakeMove(address player,uint256 position) public returns (string message)
{
    ...
    if(currentTurn)
    {
      ...
      {
      currentGameSession.CurrentBoard.InternalBoard[position]= Node(position,false,true,2);
      currentGameSession.P2.UnplacedPieces--;
      currentGameSession.P2.PlacedPieces++;
      currentGameSession.Turn=true;//Player 2's turn
